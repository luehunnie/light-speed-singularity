...
> 截止日期：2026 年 9 月 5 日
> 团队：luehunnie、qingfengdengying、youki-creat
...
- 模块解耦；
- GUI网格吸附；
- 八方向固定发射器（RAY/PARTICLE 均已八方向且已接正式运行时）；
- 开始运行按钮；
- 普通光线；
- 光粒；
- 基础镜面；
...
阶段内开发顺序（D2.6 起）：D2.6 文档重扫 → D3A `GridPlacedObject` 位置契约重构 → D3B `EmitterConfigNode` → D3C `EmissionPreview` 与真实光线接线 → D4 `BasicCrystal` 接入 → D5 模板和四层 TileMapLayer → D6 `LevelValidator` → D7A 美术配置插件 → D7C youki-creat GUI 验收 → D8 解耦、回归与文档收口。（D7B 关卡工具插件 / 第二正式 EditorPlugin 已永久取消，不延期。）
...
- D6 `LevelValidator` v0：已完成（代码侧 `COMPLETE`，PR #45–#48）。只读结构化校验器，不生成 Tile、不补画、不修改/保存场景、不依赖 `addons/`、不恢复第二 EditorPlugin；Runtime 已自动调用（D7-1 `RuntimeValidationGate` 经 D7-3 Start Run 正式入口，运行前自动校验）；
...
- 统一 `EmissionRequest` / `LightRuntimeCoordinator` / 独立光粒模块（~~光线/光粒形态切换~~ 已实装：M4-E4 `Q` 切换；~~Particle Runtime~~ 已实装：D7-4 `ParticleScheduler`，八方向）；
- ~~Particle 八方向~~（已实装：`ParticleDirection` 八方向，旧四正方向数值冻结）；
- 发射器八方向实时控制（按键已按主发射器 v0.3 冻结为 `W` 顺时针循环，代码尚无输入接线）与对应提示 UI；
- ~~真实 `RuntimeSnapshot` 玩法自动采样和自动保存（当前仅支持调用方提供数据 → 序列化/保存）~~（已实装：D7-R1 真实玩法只读采样 `runtime_snapshot_sampler` + 手动写盘；自动/每帧采样与自动保存刻意不做）；
- 稳定 `crystal_id`、`LevelObjectRegistry`、多水晶统一注册的完整接线（基础已就绪，完整多水晶正式关卡未完成）；
- 完整 `ObjectiveController` 接线；
- ~~`READY_TO_FIRE` 状态与正式“开始运行”入口~~（已实装：D7-2/D7-3 五态 + `begin_runtime`/`request_begin_runtime`）；
- ~~Runtime 自动 Validation Gate~~（已实装：D7-1 `RuntimeValidationGate` 经 D7-3 Start Run 正式调用）；
- 正式主界面、选关、最小存档、成就展示和 Windows 发布包；
- ~~Debug 游戏内控制台~~（已实装：D7-R1 Debug 只读控制台 `debug_console_view`，F3 开关；`log`/`check`/`state`/`clearlog` 命令集仍为目标）。
...
- 统一 `EmissionRequest` / `LightForm` / 光运行公共层（~~独立光粒（Particle）模块与 Particle Runtime~~ 已实装：D7-4 `ParticleScheduler`/`ParticleMotionRules`/`ParticleStepExecutor`，八方向）；
- 玩家实际八方向运行控制（按键已按主发射器 v0.3 冻结为 `W` 顺时针循环切换，代码尚无输入接线）；
- 多 Crystal 正式关卡合同、新机关批次、完整提示 UI（~~真实 `RuntimeSnapshot` 采样~~ 已实装：D7-R1）。

（~~`READY_TO_FIRE` 状态与正式“开始运行”入口~~、~~Runtime 自动 Validation Gate~~、~~玩家实际 RAY/PARTICLE 切换~~、~~Particle 八方向~~ 均已实装：D7-1/D7-2/D7-3、M4-E4。）
...
- luehunnie：核心架构、网格、运行控制、编辑器工具、跨模块集成、规范与最终审查；
- qingfengdengying：在冻结接口和对象模板内开发机关及机关测试；
- youki-creat：使用正式编辑器工作流制作关卡、替换美术、配置状态图片和对象布局。

边界：

- 不得写成luehunnie负责实现所有机关；
- qingfengdengying不得修改核心坐标、运行状态或放置协议；
- youki-creat不需要编写代码或维护复杂底层 Inspector 数据。
...
10. `GridPlacedObject` 位置契约重构（D3A，已完成：position 为场景编辑事实，cell 由 position 派生）、`EmitterConfigNode`（D3B，已完成）、`EmissionPreview` 与真实光线接线（D3C，已完成）、`BasicCrystal` 接入（D4，基础视觉与状态接口已完成）、美术配置插件（D7A，已完成，单一 `EditorPlugin`）：已完成；模板和四层 TileMapLayer（D5，已完成）、`LevelValidator` v0（D6，已完成，只读结构化校验器）、GUI 验收（D7C，Godot 原生 GUI 关卡编辑基础已完成）、编辑器方法 B（已完成，2D 拖动、64×64 吸附、写回逻辑 `cell`）：已完成；关卡工具插件（D7B / 第二正式 EditorPlugin）已永久取消，不延期；~~`READY_TO_FIRE`、开始运行入口、光线/光粒切换、光粒（Particle Runtime）、Runtime 自动 Validation Gate~~（均已实装：D7-1/D7-2/D7-3 五态与正式入口、D7-4 光粒运行时、M4-E4 `Q` 切换）；统一光运行架构：后续未实现（~~真实 `RuntimeSnapshot` 采样~~ 已实装：D7-R1）。

阶段顺序：D2.6 文档重扫 → D3A `GridPlacedObject` 位置契约重构 → D3B `EmitterConfigNode` → D3C `EmissionPreview` 与真实光线接线 → D4 `BasicCrystal` 接入 → D5 模板和四层 TileMapLayer → D6 `LevelValidator` → D7A 美术配置插件 → D7C youki-creat GUI 验收 → D8 解耦、回归与文档收口。其中 D3A、D3B、D3C、D4、D7A、D5、D6 均已完成，D7C（Godot 原生 GUI 关卡编辑基础）已完成。D7B 关卡工具插件 / 第二正式 EditorPlugin 已永久取消，不延期。Godot 原生负责节点拖动、复制、64×64 网格吸附和 position Undo/Redo，不另建吸附插件；运行时代码不得依赖 `addons/`，插件全部关闭后游戏正常运行。

第 2 项已作为 foundation 落地，后续 Diagnostics 相关增强在现有 foundation 上补强，不重新从零实现。第 3–9 项已完成行为保持式迁出，不代表 9 月 5 日交付或完整统一光运行架构完成；~~`READY_TO_FIRE`、开始运行入口、光线/光粒切换、光粒（Particle Runtime）、Runtime 自动 Validation Gate~~（均已实装：D7-1/D7-2/D7-3、D7-4、M4-E4）；统一光运行架构仍未实现（~~真实 `RuntimeSnapshot` 采样~~ 已实装：D7-R1；编辑器方法 B、D5 模板与四层 TileMapLayer、D6 `LevelValidator` v0 已完成）。`core_loop_prototype.gd`、`runtime_logger.gd`、`runtime_snapshot.gd` 当前物理行较大，但职责审查通过，禁止仅按行数机械拆分。
...
- ~~真实 RuntimeSnapshot 玩法采样~~（已实装：D7-R1）；
- ~~Debug 游戏内控制台~~（已实装：D7-R1 只读控制台；命令集仍为目标）；
- 更多自检项；
...
| 附加机关过多 | 高 | P0/P1/P2分级 |
| ~~Particle Runtime 与八方向未实现~~（已实装：B3b/M4 起 PARTICLE 八方向接正式运行时） | ~~高~~ 已消除 | M4-F1 状态同步：Particle 已八方向且已接运行时，风险条目保留为历史记录 |
| 独立对象与地图级容器锚点语义需统一 | 中 | 落地单格/多格锚点规范，明确锚点格 |
| Profile 独立副本能力尚未实现 | 中 | 后续补一键创建独立 Profile 副本 |
| ~~RuntimeSnapshot 真实采样依赖尚未成立~~（已消除：D7-R1 只读采样落地） | ~~中~~ 已消除 | 稳定 `crystal_id` 与注册基础已就绪并被采样使用；自动采样仍刻意不做 |
