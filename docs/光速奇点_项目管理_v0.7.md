...
- 关卡编辑辅助能力：方法 A 可用，方法 B 尚未完成。

## 未完成或延后

- 编辑器方法 B：2D 视图拖动、64×64 吸附、写回逻辑 `cell`——本阶段必须目标，当前尚未完成；
- 一键创建独立 Profile 副本、共享 Profile 自动转实例独立资源；
- 光线/光粒形态切换、统一 `EmissionRequest` / `LightRuntimeCoordinator` / 独立光粒模块；
- 发射器 WASD 四方向实时控制与对应提示 UI；
...
第一优先：独立可复用对象的 `PackedScene`、网格锚点和统一场景模板边界。

随后：编辑器方法 B——2D 拖动、64×64 吸附、写回逻辑 `cell`，并保证移动父节点时对象视觉和光路配置同步。
...
10. `GridPlacedObject` 位置契约重构（D3A，已完成：position 为场景编辑事实，cell 由 position 派生）、`EmitterConfigNode`（D3B，已完成）、`EmissionPreview` 与真实光线接线（D3C，已完成）、`BasicCrystal` 接入（D4，基础视觉与状态接口已完成）、美术配置插件（D7A，已完成，单一 `EditorPlugin`）：已完成；模板和四层 TileMapLayer（D5）、`LevelValidator`（D6）、关卡工具插件（D7B）、GUI 验收（D7C）、编辑器方法 B（2D 拖动、64×64 吸附、写回逻辑 `cell`）、`READY_TO_FIRE`、开始运行按钮、统一光运行架构、光线/光粒切换、光粒、真实 `RuntimeSnapshot` 采样：后续。

阶段顺序：D2.6 文档重扫 → D3A `GridPlacedObject` 位置契约重构 → D3B `EmitterConfigNode` → D3C `EmissionPreview` 与真实光线接线 → D4 `BasicCrystal` 接入 → D5 模板和四层 TileMapLayer → D6 `LevelValidator` → D7A 美术配置插件 → D7B 关卡工具插件 → D7C 张梓涵 GUI 验收 → D8 解耦、回归与文档收口。其中 D3A、D3B、D3C、D4、D7A 已完成。Godot 原生负责节点拖动、复制、64×64 网格吸附和 position Undo/Redo，不另建吸附插件；运行时代码不得依赖 `addons/`，插件全部关闭后游戏正常运行。

第 2 项已作为 foundation 落地，后续 Diagnostics 相关增强在现有 foundation 上补强，不重新从零实现。第 3–9 项已完成行为保持式迁出，不代表 9 月 5 日交付或完整统一光运行架构完成；`READY_TO_FIRE`、开始运行按钮、编辑器方法 B、光线/光粒切换、光粒、真实 `RuntimeSnapshot` 采样等仍未实现。`core_loop_prototype.gd`、`runtime_logger.gd`、`runtime_snapshot.gd` 当前物理行较大，但职责审查通过，禁止仅按行数机械拆分。
...
| 附加机关过多 | 高 | P0/P1/P2分级 |
| 编辑器方法 B 尚未完成 | 高 | 2D 拖动、64×64 吸附、写回逻辑 `cell` 列为下一开发入口 |
