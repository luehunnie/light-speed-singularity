# 《光速奇点》文档索引

> 更新日期：2026 年 7 月 29 日
> 实际文件名：`README_文档索引.md`，不得创建或引用 `.mdz` 变体。
> 本索引用于快速定位当前仓库正式文档。`docs/ai/` 只用于本地 AI 协作，不属于正式仓库文档。

---

## 1. 正式核心文档

以下文档位于 `docs/` 根目录，是当前团队共同遵守的正式核心文档：

| 文档 | 用途与当前状态 |
|---|---|
| [《光速奇点》玩法设计 v0.9](./光速奇点_玩法设计_v0.9.md) | 玩家行为、光线/光粒规则、机关、水晶、目标、关卡结构与重置规则。是已冻结设计，不等于全部已实现 |
| [《光速奇点》开发规范 v0.9](./光速奇点_开发规范_v0.9.md) | 当前代码边界、坐标、对象、Diagnostics、测试与 Git/AI 协作规范 |
| [《光速奇点》技术栈 v0.9](./光速奇点_技术栈_v0.9.md) | 当前已用技术、组件状态和实现边界 |
| [《光速奇点》项目管理 v0.7](./光速奇点_项目管理_v0.7.md) | 当前阶段、进度、风险与下一开发入口 |
| [《光速奇点》实际业务 v0.7](./光速奇点_实际业务_v0.7.md) | 交付目标和业务范围 |
| [《光速奇点》各阶段团队分工与交付物 v0.9](./光速奇点_各阶段团队分工与交付物_v0.9.md) | luehunnie、qingfengdengying、youki-creat三人职责和阶段交付 |

---

## 2. 操作指南

`docs/guides/` 保存具体操作指南：

| 文档 | 用途 |
|---|---|
| [《光速奇点》Git 分支命名规范](./guides/光速奇点_Git分支命名规范.md) | 分支类型、成员标识、命名格式和协作边界 |
| [《光速奇点》美术资源替换与 Godot 关卡编辑指南 v1.1](./guides/光速奇点_美术资源替换与Godot关卡编辑指南_v1.1.md) | 面向美术与关卡制作人员的素材替换、Profile 配置和 Godot 关卡编辑流程 |
| [《光速奇点》文件结构与模块调度链 v1.1](./guides/光速奇点_文件结构与模块调度链_v1.1.md) | 目录职责、模块边界、依赖方向、主要调度链与迁移顺序 |

---

## 3. API 契约

`docs/api/` 保存公共接口、长期契约和技术约定：

| 文档 | 用途 |
|---|---|
| [《光速奇点》永久视觉与关卡编辑接口设计 v1.1](./api/光速奇点_永久视觉与关卡编辑接口设计_v1.1.md) | 视觉 Profile、对象视觉组件、GridPlacedObject、关卡编辑和资源替换接口契约 |

> 注意：
> - 接口设计文档包含长期目标契约；
> - `GridPlacedObject` 基类已实现；编辑器方法 B 单格路径已完成（D5），多格对象编辑流程仍为目标；
> - 不得仅凭 API 文档判断功能已经可用；
> - 实施状态必须结合真实代码和当前状态文档。

---

## 4. 本地 AI 协作目录

`docs/ai/` 只用于本地 AI 协作上下文、当前状态、下一步任务计划和测试清单：

- 该目录被 `.gitignore` 忽略；
- 不属于正式仓库文档；
- 不替代 `docs/` 根目录、`docs/api/` 和 `docs/guides/` 中的正式文档；
- `docs/ai/archive/` 只作历史参考；
- archive 内容不得覆盖正式文档。

---

## 5. 当前实现摘要与下一开发方向

当前实现摘要：

已完成：

- `GridPlacedObject`、`EmitterConfigNode`、`EmissionPreview`；
- 当前光线路径接线（发射器真实光路端到端集成测试已落地）；
- `BasicCrystal` 基础视觉与状态接口；
- Placement 与 DragFlow 基础（`PlacementController` / `InventoryController` / `DragFlowController` / `OccupancyRegistry`）；
- Runtime 与 Diagnostics 基础（`RunStateController` / `LevelRuntimeController` / `DiagnosticsController` / `RuntimeLogger` / `RuntimeSnapshotData` / `StartupSelfCheckCoordinator`）；
- 单一美术 Profile 插件 `addons/light_speed_art_profile/`（Dock、Panel、Service、Resolver、Browser 均为该插件内部模块）；
- 核心测试架构按职责拆分；
- 回收事务一致性修复。

未完成：

- ~~编辑器方法 B（2D 拖动、64×64 吸附、写回逻辑 `cell`）~~（已实装：D5）；
- Profile 独立副本与共享 Profile 自动转实例独立资源；
- ~~光线/光粒形态切换、统一光运行架构、光粒~~（形态切换已实装 M4-E4 `Q`；光粒运行时已实装 D7-4/M4；统一光运行架构仍为目标）；
- WASD 发射器实时控制与提示 UI；
- 真实 `RuntimeSnapshot` 自动采样与自动保存（只读采样与手动写盘已实装 D7-R1，自动/每帧采样刻意不做）；
- ~~`READY_TO_FIRE` 状态与开始运行按钮~~（已实装：D7-2/D7-3 五态 + 正式入口）；
- 正式主界面、选关、存档、成就与 Windows 发布流程。

当前下一开发方向：

- 独立可复用对象的 `PackedScene` 模板；
- ~~单格/多格锚点规范落地~~（已实装：D7-R4 多格占用基础接口，anchor/occupied-cells 契约）；
- 编辑器方法 B（2D 拖动、64×64 吸附、写回逻辑 `cell`，移动父节点时视觉和光路配置同步）；
- 在冻结接口和对象模板内开发机关和关卡内容。

D8 Diagnostics 保持现有架构，在 foundation 上补强，不作为大规模重构任务。

---

## 6. 优先级说明

发生冲突时使用以下顺序：

1. 用户在当前任务中的明确决定；
2. 当前仓库真实 Git、代码、场景、资源和实际运行结果；
3. 当前阶段交接说明或阶段总文档；
4. 当前正式核心文档；
5. API 契约；
6. 操作指南；
7. `docs/ai/` 当前协作文档；
8. `docs/ai/archive/` 历史归档。

补充：

- 目标设计不能覆盖真实实施状态；
- archive 不能覆盖当前文档；
- 无法确认时必须记录为待确认。
