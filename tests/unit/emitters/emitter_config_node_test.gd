...
## EmitterConfigNode D3B-1 / D3C-0 / D3C-2.5 定向自动测试。
...
##   不创建缺失子节点。
##   D3C-2.5：EmitterVisual.rotation 由 default_light_form 与活动方向驱动（RIGHT=0、DOWN_RIGHT=PI/4、DOWN=PI/2、
##   UP=-PI/2、LEFT≈±PI）、PARTICLE 用光粒方向、两形态方向独立保存、切换形态立即更新、非活动方向不改当前朝向、
##   根节点 rotation 始终 0、EmitterVisual.position 不变、重复值不刷新朝向。
