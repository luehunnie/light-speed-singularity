...
## LevelRuntimeController 单元测试（拆分片 4/5 · 运行期移动次数与交互；D7-2 经 READY_TO_FIRE 发射）。
## 覆盖：SETUP 跨格不扣、READY_TO_FIRE/PULSE_ACTIVE/MOVE_WINDOW 跨格扣一次、原格/取消/回收/新放置不扣、达上限拒绝提交。
## D7-2 起 SETUP 不可直接发射，运行期用例先 begin_runtime 进入 READY_TO_FIRE 再 begin_pulse；READY 跨格移动同属运行期计次。
...
## 21. READY/PULSE/MOVE 跨格扣一次：三态下跨格 consume 均返回 true 且 used 递增（D7-2 READY 同属运行期计次状态）。
func _test_21_runtime_move_consumes_once() -> void:
	const NAME: String = "21_运行期跨格扣一次"
	var env: _Fixture._Env = _fixture.make_env(Vector2i(1, 3), Vector2i.RIGHT, null, 3)
	# READY_TO_FIRE 跨格移动按运行期规则计次（D7-2 新增）。
	env.rsc.begin_runtime()
	_check(NAME, env.rsc.get_current_state() == _RuntimeInteractionTypes.RunState.READY_TO_FIRE, "前置应 READY_TO_FIRE。")
	_check(NAME, env.controller.consume_runtime_move_if_required(Vector2i(1, 1), Vector2i(2, 1)), "READY_TO_FIRE 跨格应扣次。")
	# PULSE_ACTIVE 跨格扣次。
	env.rsc.begin_pulse()
	_check(NAME, env.controller.consume_runtime_move_if_required(Vector2i(2, 1), Vector2i(3, 1)), "PULSE_ACTIVE 跨格应扣次。")
	...
	_check(NAME, env.controller.consume_runtime_move_if_required(Vector2i(3, 1), Vector2i(4, 1)), "MOVE_WINDOW 跨格应扣次。")
	_check(NAME, env.controller.get_runtime_moves_used() == 3, "used 期望 3。")
