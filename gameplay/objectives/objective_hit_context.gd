...
## RAY 命中额外携带 color（到达色 ColorValue；PARTICLE 命中恒为 NONE 哨兵——光粒不带颜色，玩法设计 §6/§7）。
...
var _speed_tier: int
## 到达颜色（RayColor.ColorValue 值）；RAY 命中为真实四色（WHITE/RED/GREEN/BLUE），PARTICLE 命中恒为 NONE（光粒无颜色）。
var _color: int


## 构造 Ray 形态命中事实；非法方向或非法颜色（须为真实四色）返回 null 并 push_error。
static func create_for_ray(
	...
	return _create(cell, incoming_direction, _LightEmissionTypes.LightForm.RAY, emission_id, runtime_generation, RAY_SPEED_TIER_NONE, color)


## 构造 Particle 形态命中事实；非法方向或非法速度档位返回 null 并 push_error（光粒不带颜色，颜色恒为 NONE）。
static func create_for_particle(
	...
	return _create(cell, incoming_direction, _LightEmissionTypes.LightForm.PARTICLE, emission_id, runtime_generation, speed_tier, _RayColor.ColorValue.NONE)


## 统一构造入口（私有）：校验方向 / 形态 / 速度档位 / 颜色后一次写入全部字段。
static func _create(
		cell: Vector2i,
		incoming_direction: Vector2i,
		light_form: int,
	...
	if light_form == _LightEmissionTypes.LightForm.PARTICLE:
		if speed_tier < _ParticleMotionRules.SpeedTier.SLOW or speed_tier > _ParticleMotionRules.SpeedTier.FAST:
			...
			return null
		if color != _RayColor.ColorValue.NONE:
			push_error("ObjectiveHitContext：PARTICLE 命中不得携带颜色 %d，拒绝构造。" % [color])
			return null
	elif speed_tier != RAY_SPEED_TIER_NONE:
		push_error("ObjectiveHitContext：RAY 命中不得携带速度档位 %d，拒绝构造。" % [speed_tier])
		return null
	else:
		if not _RayColor.is_valid(color):
			push_error("ObjectiveHitContext：RAY 命中携带非法颜色 %d，拒绝构造。" % [color])
			return null
...
## 到达颜色（RayColor.ColorValue 值；RAY 命中为真实四色，PARTICLE 命中为 NONE 哨兵，只读）。
func get_color() -> int:
	return _color
