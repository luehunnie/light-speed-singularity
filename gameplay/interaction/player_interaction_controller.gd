...
## 不查询 RunState、不命中 UI、不转网格坐标、不开始/结束拖拽、不调用库存/PlacementController、不发射光线、不执行 R、不修改 Node、不调用 set_input_as_handled、不访问场景树。
## 按键释放、无关键与无关鼠标键统一返回 NONE；FIRE/RESET 经 fire_light/reset_level 输入动作识别，与核心原有一致。
