...
## 运行状态纯规则共享模块（批次 4B-F2）。
## 职责：集中持有 core_loop_prototype.gd 原有的六条依赖 RunState 的纯状态权限规则与脉冲结束状态计算，
...
## 调用方（core_loop_prototype）仍负责 _set_run_state 事务、UI 刷新、拖拽取消、发射流程、R 重置与关卡完成逻辑；
...
## 本批不修正任何现有行为，六条规则与迁移前实现严格等价，不增加 READY_TO_FIRE 等新状态或新规则。
...
## [br]职责：判定 Space 发射权限。
## [br]输入：state 是当前运行状态。
## [br]返回：true 表示 SETUP 或 MOVE_WINDOW 可以发射；false 表示 PULSE_ACTIVE 或 COMPLETED 必须拒绝 Space。
...
static func can_fire_light(
	...
	return state == _RuntimeInteractionTypes.RunState.SETUP or state == _RuntimeInteractionTypes.RunState.MOVE_WINDOW
...
## [br]职责：判定当前是否处于会消耗运行期移动次数的状态（PULSE_ACTIVE 或 MOVE_WINDOW）。
## [br]输入：state 是当前运行状态。
## [br]返回：true 表示当前处于 PULSE_ACTIVE 或 MOVE_WINDOW；SETUP 与 COMPLETED 返回 false。
## [br]副作用：无；不读取或修改任何实例状态，不修改输入枚举。
## [br]失败：不会失败；任意 RunState 输入均返回确定布尔结果。
## [br]边界：运行期移动次数只在 PULSE_ACTIVE 和 MOVE_WINDOW 中扣除，SETUP 移动不计次，COMPLETED 冻结全部布局交互；
## 是否真正扣次由 RuntimeMoveRules 与调用方在占用原子更新成功后决定，本函数只判定状态归属，不执行状态切换。
static func is_runtime_move_state(
		state: _RuntimeInteractionTypes.RunState
) -> bool:
	return state == _RuntimeInteractionTypes.RunState.PULSE_ACTIVE or state == _RuntimeInteractionTypes.RunState.MOVE_WINDOW
